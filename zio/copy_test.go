...
func TestCopy(t *testing.T) {
	...
	t.Run("read write success", func(t *testing.T) {
		...
		ztesting.AssertEqual(t, 10, written)
	...
	t.Run("reader EOF", func(t *testing.T) {
		...
		w := bytes.NewBuffer(nil)
		written, err := zio.Copy(w, r)
		ztesting.AssertEqual(t, 5, written)
		ztesting.AssertEqual(t, nil, err)
	})
	...
	t.Run("reader UnexpectedEOF", func(t *testing.T) {
		...
		written, err := zio.Copy(w, r)
		ztesting.AssertEqual(t, 5, written)
		ztesting.AssertEqual(t, io.ErrUnexpectedEOF, err)
	})
	...
	t.Run("read write error", func(t *testing.T) {
		...
		written, err := zio.Copy(w, r)
		ztesting.AssertEqual(t, 4, written) // Write error is returned.
		ztesting.AssertEqual(t, io.ErrClosedPipe, err)
	})
	...
	t.Run("nRead != nWrite", func(t *testing.T) {
		...
		written, err := zio.Copy(w, r)
		ztesting.AssertEqual(t, 4, written)
		ztesting.AssertEqual(t, io.ErrShortWrite, err)
	})
}
