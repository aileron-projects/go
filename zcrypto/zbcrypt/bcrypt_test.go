...
func TestNew(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		h, _ := b.Sum([]byte("test"))
		ztesting.AssertEqual(t, "$2a$10$Lxe3KBCwKxOzLha2MR.vKePt0LpiRJVu6NMWU/u7ddWJx0sm6kBb.", string(h))
	})
	t.Run("min cost", func(t *testing.T) {
		...
		ztesting.AssertEqualErr(t, nil, err)
		h, _ := b.Sum([]byte("test"))
		ztesting.AssertEqual(t, "$2a$04$Lxe3KBCwKxOzLha2MR.vKeIyr1H1Cb4ubhc770bjAQSAVlORxnG2S", string(h))
	})
	t.Run("param invalid cost=32", func(t *testing.T) {
		b, err := zbcrypt.New(32)
		ztesting.AssertEqual(t, nil, b)
		ztesting.AssertEqualErr(t, bcrypt.InvalidCostError(32), err)
	})
}
...
func TestCompare(t *testing.T) {
	...
	ztesting.AssertEqualErr(t, nil, err)
	h, _ := b.Sum([]byte("test"))
	ztesting.AssertEqualErr(t, nil, b.Compare(h, []byte("test")))
	ztesting.AssertEqualErr(t, bcrypt.ErrMismatchedHashAndPassword, b.Compare(h, []byte("wrong")))
}
