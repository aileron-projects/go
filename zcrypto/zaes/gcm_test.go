...
func TestGCM(t *testing.T) {
	...
	t.Run("encrypt key invalid", func(t *testing.T) {
		key := []byte("short")
		ciphertext, err := zaes.EncryptGCM(key, nil)
	...
	t.Run("AES128: encrypt-decrypt empty", func(t *testing.T) {
		...
		plaintext, err := zaes.DecryptGCM(key, ciphertext)
		ztesting.AssertEqualErr(t, "error not match", nil, err)
		ztesting.AssertEqual(t, "plaintext does not match", nil, plaintext)
	})
	t.Run("AES128: encrypt-decrypt", func(t *testing.T) {
		key := []byte("1234567890123456")
		msg := []byte("test message")
		ciphertext, err := zaes.EncryptGCM(key, msg)
		ztesting.AssertEqualErr(t, "error not match", nil, err)
		plaintext, err := zaes.DecryptGCM(key, ciphertext)
		ztesting.AssertEqualErr(t, "error not match", nil, err)
