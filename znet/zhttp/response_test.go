...
func TestWrapResponseWriter(t *testing.T) {
	t.Run("new wrapper", func(t *testing.T) {
		...
		ztesting.AssertEqual(t, "status code not match", http.StatusBadRequest, w.Result().StatusCode)
	})
	t.Run("already wrapped", func(t *testing.T) {
		w := httptest.NewRecorder()
		ww1 := zhttp.WrapResponseWriter(w)
		ww2 := zhttp.WrapResponseWriter(ww1)
		ztesting.AssertEqual(t, "writer not match", ww1, ww2)
...
func TestResponseWrapper(t *testing.T) {
	t.Run("not written", func(t *testing.T) {
		...
		ztesting.AssertEqual(t, "written bytes not match", 0, ww.Written())
	})
	...
	t.Run("non-nil body written", func(t *testing.T) {
		...
		ww.Write([]byte("test"))
		ztesting.AssertEqual(t, "status code not match", http.StatusOK, ww.StatusCode())
		ztesting.AssertEqual(t, "written bytes not match", 4, ww.Written())
	})
	t.Run("hook on status written", func(t *testing.T) {
		...
		ww.WriteHeader(http.StatusBadRequest)
		ztesting.AssertEqual(t, "status code not match", http.StatusBadRequest, status)
	})
	t.Run("hook on body written", func(t *testing.T) {
		...
		ww.StatusWritten = func(statusCode int) { status = statusCode }
		ww.Write(nil)
		ztesting.AssertEqual(t, "status code not match", http.StatusOK, status)
	})
	t.Run("hijack error", func(t *testing.T) {
		w := httptest.NewRecorder()
		ww := zhttp.WrapResponseWriter(w)
		_, _, err := ww.Hijack()
		ztesting.AssertEqual(t, "hijack should be error", true, err != nil)
	})
}
