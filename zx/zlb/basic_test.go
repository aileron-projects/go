...
var basicTargets = map[string]struct {
	found   bool
	names   []string
	targets []*Target
}{
	...
	"2 targets, non-0 weight": {
		found: true,
		names: []string{"t0", "t1", "t1", "t0", "t1", "t1"},
		targets: []*Target{
			{name: "t0", id: 0, weight: 1, active: true},
	...
	"2 targets, equal weight": {
		found: true,
		names: []string{"t0", "t0", "t1", "t1", "t0", "t0", "t1", "t1"},
		targets: []*Target{
	...
	"3 targets, weights 321": {
		found: true,
		names: []string{"t0", "t0", "t0", "t1", "t1", "t2"},
		targets: []*Target{
	...
	"3 targets, weights 231": {
		found: true,
		names: []string{"t0", "t0", "t1", "t1", "t1", "t2"},
		targets: []*Target{
			{name: "t0", id: 0, weight: 2, active: true},
	...
	"3 targets, weights 123": {
		found: true,
		names: []string{"t0", "t1", "t1", "t2", "t2", "t2"},
		targets: []*Target{
			{name: "t0", id: 0, weight: 1, active: true},
			{name: "t1", id: 1, weight: 2, active: true},
...
func TestBasicRoundRobin_Get(t *testing.T) {
	t.Parallel()
	for name, tc := range basicTargets {
...
func TestBasicRoundRobin_Remove(t *testing.T) {
	t.Parallel()
	...
	t.Run("remove i<current", func(t *testing.T) {
		t0 := &Target{name: "t0", id: 0, weight: 1, active: true}
		t1 := &Target{name: "t1", id: 1, weight: 1, active: true}
		t2 := &Target{name: "t2", id: 2, weight: 2, active: true}
		lb := zlb.NewBasicRoundRobin(t0, t1, t2)
