...
func TestBaseLB_Add(t *testing.T) {
	...
	t.Run("add one", func(t *testing.T) {
		...
		lb.Add(t1)
		ts := lb.Targets()
		ztesting.AssertEqual(t, 1, len(ts))
		ztesting.AssertEqual(t, "t1", ts[0].name)
	})
	t.Run("add multiple", func(t *testing.T) {
		lb := &baseLB[*testTarget]{}
		lb.Add(t1, t2, t3)
		ts := lb.Targets()
		ztesting.AssertEqual(t, 3, len(ts))
		ztesting.AssertEqual(t, "t1", ts[0].name)
		ztesting.AssertEqual(t, "t2", ts[1].name)
		ztesting.AssertEqual(t, "t3", ts[2].name)
	})
}
...
func TestBaseLB_Remove(t *testing.T) {
	...
	t.Run("mode0", func(t *testing.T) {
		t.Run("remove from empty", func(t *testing.T) {
			lb := &baseLB[*testTarget]{}
			lb.Remove(1)
			ztesting.AssertEqual(t, 0, len(lb.targets))
		})
		t.Run("remove not match", func(t *testing.T) {
			lb := &baseLB[*testTarget]{}
			lb.Add(t1, t2, t3)
			lb.Remove(999)
			ztesting.AssertEqual(t, 3, len(lb.targets))
		})
		t.Run("remove first", func(t *testing.T) {
			...
			lb.Remove(1)
			ts := lb.Targets()
			ztesting.AssertEqual(t, 2, len(ts))
			ztesting.AssertEqual(t, "t2", ts[0].name)
			ztesting.AssertEqual(t, "t3", ts[1].name)
		})
		...
		t.Run("remove complex", func(t *testing.T) {
			...
			lb.Remove(2)
			ts := lb.Targets()
			ztesting.AssertEqual(t, 4, len(ts))
			ztesting.AssertEqual(t, "t1", ts[0].name)
			ztesting.AssertEqual(t, "t3", ts[1].name)
			ztesting.AssertEqual(t, "t1", ts[2].name)
			ztesting.AssertEqual(t, "t3", ts[3].name)
		})
	})
	...
	t.Run("mode1", func(t *testing.T) {
		...
		t.Run("remove first", func(t *testing.T) {
			...
			ts := lb.Targets()
			ztesting.AssertEqual(t, 2, len(ts))
			ztesting.AssertEqual(t, "t3", ts[0].name)
			ztesting.AssertEqual(t, "t2", ts[1].name)
		})
		t.Run("remove middle", func(t *testing.T) {
			lb := &baseLB[*testTarget]{removeMode: 1}
		...
		t.Run("remove complex", func(t *testing.T) {
			...
			ztesting.AssertEqual(t, "t3", ts[1].name)
			ztesting.AssertEqual(t, "t3", ts[2].name)
			ztesting.AssertEqual(t, "t1", ts[3].name)
		})
	})
}
