...
func TestBucket(t *testing.T) {
	...
	t.Run("bucketSize=-1", func(t *testing.T) {
		...
		ztesting.AssertEqual(t, math.MaxInt64, after)
	})
	t.Run("bucketSize=0", func(t *testing.T) {
		getToken := getTokenFunc(0, 10, time.Second, time.Now)
		ok, after := getToken()
	...
	t.Run("interval=-1", func(t *testing.T) {
		...
		ztesting.AssertEqual(t, 0, after)
	})
	...
	t.Run("fillRate=-1", func(t *testing.T) {
		...
		ok1, after1 := getToken()
		ztesting.AssertEqual(t, true, ok1)
		ztesting.AssertEqual(t, 0, after1)
		ok2, after2 := getToken()
		ztesting.AssertEqual(t, false, ok2)
		ztesting.AssertEqual(t, time.Hour, after2)
	})
	...
	t.Run("fillRate=1", func(t *testing.T) {
		...
		*now = (*now).Add(time.Second) // Forward 1 sec.
		ok2, after2 := getToken()
		ztesting.AssertEqual(t, true, ok2)
		ztesting.AssertEqual(t, 0, after2)
	})
	t.Run("fillRate=1,try3times", func(t *testing.T) {
		...
		*now = (*now).Add(100 * time.Millisecond) // Forward 0.1 sec.
		ok3, after3 := getToken()
		ztesting.AssertEqual(t, false, ok3)
		ztesting.AssertEqual(t, 900*time.Millisecond, after3)
	})
	t.Run("fillRate=2", func(t *testing.T) {
		...
		ok1, after1 := getToken()
		ztesting.AssertEqual(t, false, ok1)
		ztesting.AssertEqual(t, time.Second, after1)
		*now = (*now).Add(time.Second) // Forward 1 sec.
		ok2, after2 := getToken()
		ztesting.AssertEqual(t, true, ok2)
		ztesting.AssertEqual(t, 0, after2)
		ok3, after3 := getToken()
		ztesting.AssertEqual(t, true, ok3)
		ztesting.AssertEqual(t, 0, after3)
	})
}
